...
:setJexec
    "%SEARCH4J_EXECUTABLE%" -e -m "%MIN_VERSION%" -p "%HINT%" | "%GSDLHOME%\bin\windows\setvar.exe" JAVA_EXECUTABLE > %TMP%\set_java_executable.bat
    ...
    "%SEARCH4J_EXECUTABLE%" -m "%MIN_VERSION%" -p "%HINT%" | "%GSDLHOME%\bin\windows\setvar.exe" GS_JAVA_HOME > %TMP%\set_java_home.bat
