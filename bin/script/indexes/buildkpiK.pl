...
#usage: perl buildkpiK.pl [-R] [collection] [collection] etc
#
...
#-From each file collect kea phrases. 
#-Determine the number of kea phrases for each document 
...
#the kea data, and then searches for these phrases in the file, counting and storing 
#how many times each phrase appears. The data is then sent to function keyphrase_document 
#with arguments hash ID, kea phrases to build the keyphrase_document index.
#The function which builds the document_keyphrase index is then passed the hash ID, the kea 
#phrases and the array/s which hold the number of times each phrase
...
sub build_index {
	...
	#using regular expressions generated from kea-reg 
...
#This function is passed as arguments file hash ID and a list of kea phrases
...
sub keyphrase_document{
    ...
    my %table = keyphrase_index_search($kea); 
...
sub document_keyphrase {
    ...
    #get table of phrases and phrase indexes
    my %table = keyphrase_index_search($keaS);
