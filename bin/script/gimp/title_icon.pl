...
local ($cfg_file, $size, $imagefile, $width, $height, $imageheight, $stripecolor, $stripewidth, 
       ...
       $fontweight, $fontslant, $fontwidth, $fontspacing, $image_dir, $dont_wrap);
...
sub produce_image {
    ...
    gimp_bucket_fill ($image, $backlayer, FG_BUCKET_FILL, NORMAL_MODE, 100, 0, 1, 0, 0);
    gimp_selection_none ($image);
	...
	gimp_scale ($rimage, $rdraw, 1, 0, 0, $width-$stripewidth, $height);
	gimp_edit_copy ($rimage, $rdraw);
	...
	# clear the new layer
	gimp_selection_all ($image);
	gimp_edit_clear ($image, $imagelayer);
	gimp_selection_none ($image);
	
	my $flayer = gimp_edit_paste ($image, $imagelayer, 1); 
	if ($stripe_alignment eq "left") {
