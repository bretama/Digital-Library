...
_textdescrdeposit_ {Deposit item into the collection}
_textdescrlogin_ {Login page}
_textdescrlogout_ {Logout as _cgiargunHtmlsafe_}

_textdescrgreenstone_ {Abiyi Addi Digital Library Software}
...
_textTitle_ {Title}
_labelTitle_ {Title}
_textCreator_ {Author}
_labelCreator_ {Author}
...
_textSource_ {}  # Not true DC, kept for legacy reasons



##filename button
_labelSource_ {}  # Not true DC, kept for legacy reasons
...
###browse by original filename
_textdescrSource_ {}  # Not true DC, kept for legacy reasons
...
_linktextHOME_ {Home}
_linktextHELP_ {Help}
_linktextPREFERENCES_ {Pref}
_linktextDEPOSIT_ {DEPOSIT}
_linktextLOGIN_ {LOGIN}
_linktextLOGGEDIN_ {(logged in as _cgiargunHtmlsafe_)}
...
Helps you create collections online. This predates the Librarian Interface, and for most practical purposes the Librarian Interface should be used instead.
}
...
Helps you to add documents to existing collections online, instead of using GLI. 
}

_textdepositorlocal_{Start New Deposit}
package gti
...
# any matches
_textquerytitle_ {_If_(_thislast_,results _thisfirst_ - _thislast_ for query: _cgiargqHtmlsafe_,No matches for query: _cgiargqHtmlsafe_)}
