...
_foundintcontent_ [l=ja] {<h3>Link to "_2_" collection</h3> <p> 選択されたリンクは_collectionname_コレクションの外部にあります。（"_2_" コレクションにリンクしています）      もし"_2_" コレクションのリンクを見たければ<a href="_httpdoc_&c=_1_&cl=_cgiargclUrlsafe_&d=_3_">進む</a>このページに;      もしくはブラウザ-の「バック」ボタンを使用して以前のドキュメントへ戻ってください。}
...
_textmustbelongtogroup_ [l=ja] {このページにアクセスするには、 "_cgiargugHtmlsafe_" グループに属していなければなりません。}
...
_textdelperm_ [l=ja] {コレクションのいくつかあるいは全部が削除できません。考えられる原因としては <ul> <li>グリーンストーンは _gsdlhome_/collect/_cgiargbc1dirnameHtmlsafe_を削除する許可権限を持っていません。 directory.<br> You may need to remove this directory manually to complete the removal of the _cgiargbc1dirnameHtmlsafe_ collection from this computer.このコンピューターから_cgiargbc1dirnameHtmlsafe_の移動を完成するためにこのディレクトリーをマニュアルで移動する必要があります。</li> <li>グリーンストーンは_gsdlhome_/bin/script/delcol.plプログラムを起動できません。このファイルが読み取り可能でかつ実行可能であることを確認してください。</li> </ul>}

_textdelinv_ [l=ja] {コレクター}

_textdelsuc_ [l=ja] {_cgiargbc1dirnameHtmlsafe_ コレクションは削除されました。}

_textclonefail_ [l=ja] {_cgiargclonecolHtmlsafe_  コレクションは複製できません。考えられる原因としては： <ul> <li> The _cgiargclonecolHtmlsafe_ collection が存在しない。 <li> The _cgiargclonecolHtmlsafe_ collection が collect.cfg configuration fileを持たない。 <li> グリーンストーンがcollect.cfg configuration fileを読む権限がない。 </ul>}
...
_textaboutpassword_ [l=ja] {パスワードは3から8文字までの長さです。アスキーコードで通常印刷できるどの文字も可能です。 }

_textoldpass_ [l=ja] {このフイールドが空白なら古いパスワードが保持される。}
_textenabled_ [l=ja] {使用可能}
_textdisabled_ [l=ja] {使用不可能}

_textaboutgroups_ [l=ja] {グループはコンマで区切られたリストで、コンマのあとはスペースは置かない。}
...
_textremwarn_ [l=ja] {ユーザーを永久に削除しますか <b>_cgiargumunHtmlsafe_</b>?}
...
_textgtigsinstaller_ [l=ja] {グリーンストーンのインストーラー}
_textgtigs3colcfg_ [l=ja] {G83 デモコレクション-config strings　}
