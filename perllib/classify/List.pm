...
my $valid_meta_select_types = { 'firstvalue' => 1,
...
my $arguments = 
	...
	'type' => "metadata",
	'reqd' => "yes" },
      { 'name' => "metadata_selection_mode",
	'desc' => "{List.metadata_selection_mode}",
	'type' => "enumstring", # Must be enumstring because multiple values can be specified (separated by '/')
	'list' => $meta_select_type_list,
	'deft' => "firstvalidmetadata" },
...
sub new
{
	...
	foreach my $metadata_group (@metadata_groups) {
	    $self->{$metadata_group . ".metadata_selection_mode"} = "firstvalidmetadata";
	}
    } else {
	my @metadata_selection_mode_list = split(/\//, $self->{'metadata_selection_mode'});
	foreach my $metadata_group (@metadata_groups) {
	    my $meta_select_mode = shift(@metadata_selection_mode_list);
	    if (defined($meta_select_mode) && defined $valid_meta_select_types->{$meta_select_mode}) {
		$self->{$metadata_group . ".metadata_selection_mode"} = $meta_select_mode;
	    } else {
		$self->{$metadata_group . ".metadata_selection_mode"} = "firstvalidmetadata";	
	    }
	}
	...
	if (!defined $self->{$sort_group . ".metadata_selection_mode"}) {
	    $self->{$sort_group . ".metadata_selection_mode"} = "firstvalue";
	}
    }
...
sub classify
{
	...
	#print STDERR "filtering documents on $self->{'filter_metadata'}\n";
	...
	if ($self->{'filter_regex'} ne "" && $meta !~ /$self->{'filter_regex'}/) {
	    #print STDERR "doesn't match regex\n";
	    return;
...
sub classify_section
{
	...
	unless ($metadata_groups_done{$metadata_group}) {
		...
		foreach my $metadata_value (@metadata_values) {
		    ...
		    # We are already working with unicode aware strings at this 
		    # stage, so we no longer need to convert from utf8 to unicode 
		    #my $metadata_value_unicode_string = $metadata_value; # $self->convert_utf8_string_to_unicode_string($metadata_value);
			...
			last if ($self->{$metadata_group . ".metadata_selection_mode"} eq "firstvalue");
		    }
		} # foreach metadatavalue
		last if ((@metadata_values > 0) &&  $self->{$metadata_group . ".metadata_selection_mode"} =~ /^(firstvalue|firstvalidmetadata)$/ );
...
sub add_vlist
{
	...
	# Otherwise create a sublist (bookshelf) for the metadata value
	    ...
	    # Note that we don't need to convert from unicode-aware strings 
	    # to utf8 here, as that is handled elsewhere in the code
	    my %child_classifier_node = ( 'Title' => $metadata_value_display, # 'Title' => $self->convert_unicode_string_to_utf8_string($metadata_value),
					  'childtype' => "VList",
...
sub sort_metadata_values_array
{
	...
	return sort { $self->alpha_numeric_cmp($a,$b) }(@metadata_values);
    }
}
