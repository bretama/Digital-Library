...
# $Id: Entities.pm 22649 2010-08-17 02:03:34Z davidb $
...
# The version we (Greenstone) seem to be using looks like it's come
# from CVS/SVN and so does not parse the following sprintf statement
# => Doesn't appear to be used anywhere in this file, so commenting out
#
#$VERSION = sprintf("%d.%02d", q$Revision: 22649 $ =~ /(\d+)\.(\d+)/);
#sub Version { $VERSION; }
